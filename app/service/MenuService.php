    ...
    public function getMenuInfo($categoryId = null, $tagid = null, $date = null){
        ...
        if (!empty($date)) { // 优先查询日期的文章信息，并且再首页频道显示
            ...
            if (!empty($tag)) {
                ...
                $navigation .= '<a href="/">首页</a>&gt; <a href="/index/index?category_id=' . $fatherCategory->getId() . '"> ' . $fatherCategory->getName() . '</a>&gt; <a href="/index/index?categoryid=' . $categoryInfo->getId() . '"> ' . $categoryInfo->getName() . '</a>' . '</a>&gt; <a href="/index/index?tagid=' . $tag->getId() . '"> ' . $tag->getName() . '</a></h2>';
            ...
            if (empty($categoryInfo)) {
                ...
                $fatherCategory = $this->categoryService->getCategoryById($categoryInfo->getPid());
                $categoryid = $fatherCategory->getId();
                $navigation .= '<a href="/">首页</a>&gt; <a href="/index/index?categoryid=' . $fatherCategory->getId() . '"> ' . $fatherCategory->getName() . '</a>&gt; <a href="/index/index?categoryid=' . $categoryInfo->getId() . '">' . $categoryInfo->getName() . '</a></h2>';
            } else {
                $navigation .= '<a href="/">首页</a>&gt; <a href="/index/index?categoryid=' . $categoryInfo->getId() . '"> ' . $categoryInfo->getName() . '</a>&gt; 最新文章</h2>';

	...
	public function afterQuery(Event $event,Mysql $pdo){
		...
		foreach ($profiles as $profile) {
			...
			if ($elapsesecondes > 0.5) {
				$this->log->error('消耗时间:{},sql:{},参数信息:{}',array($elapsesecondes,$profile->getSQLStatement(),$this->params));
			} else if ($elapsesecondes > 0.2){
				$this->log->info('消耗时间:{},sql:{},参数信息:{}',array($elapsesecondes,$profile->getSQLStatement(),$this->params));
			} else {
				$this->log->debug('消耗时间:{},sql:{},参数信息:{}',array($elapsesecondes,$profile->getSQLStatement(),$this->params));
			}
		}
	}
